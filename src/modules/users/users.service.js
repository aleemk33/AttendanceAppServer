...
function validateRoleCombination(roles) {
    for (const combo of INVALID_ROLE_COMBOS) {
        if (combo.every((r) => roles.includes(r))) {
            throw new BadRequestError(`Invalid role combination: ${combo.join(' + ')} is not allowed`);
        }
    }
}
/**
...
export async function listUsers(callerRoles, callerId, filters) {
    ...
    return { items: users, meta: paginationMeta(total, filters.page, filters.limit) };
}
/**
...
export async function createUser(callerRoles, callerId, data) {
    ...
    if (data.managerUserId) {
        const manager = await prisma.user.findUnique({ where: { id: data.managerUserId } });
...
export async function updateUser(callerRoles, callerId, userId, data) {
    ...
    if (callerRoles.includes(Role.MANAGER) && !callerRoles.includes(Role.ADMIN)) {
        ...
        if (data.roles || data.managerUserId !== undefined) {
            throw new ForbiddenError('Managers cannot change roles or reassign manager');
        }
    }
    if (data.roles) {
        validateRoleCombination(data.roles);
 ...
 */
export async function updateAttendanceProfile(callerRoles, callerId, userId, data) {
    const prisma = getPrisma();
    const user = await prisma.user.findUnique({ where: { id: userId } });
    if (!user)
        throw new NotFoundError('User');
