...
import { businessToday, countWorkingDays, dateRange, isWeeklyOff, } from '../../common/index.js';
...
async function getHolidayDatesInRange(startDate, endDate) {
    ...
    for (const h of holidays) {
        const dates = dateRange(h.startDate.toISOString().slice(0, 10), h.endDate.toISOString().slice(0, 10));
...
export async function createLeaveRequest(userId, data) {
    ...
    const holidayDates = await getHolidayDatesInRange(data.startDate, data.endDate);
    const { count, workingDates } = countWorkingDays(data.startDate, data.endDate, holidayDates);
    if (count === 0) {
    ...
    if (overlapping.length > 0) {
        ...
        for (const existing of overlapping) {
            const existingDates = dateRange(existing.startDate.toISOString().slice(0, 10), existing.endDate.toISOString().slice(0, 10));
            const existingWorkingDates = existingDates.filter((d) => !isWeeklyOff(d) && !holidayDates.has(d));
            const overlap = workingDates.filter((d) => existingWorkingDates.includes(d));
            if (overlap.length > 0) {
                throw new ConflictError('Leave request overlaps with an existing pending/approved leave');
            }
        }
    }
...
export async function cancelLeaveRequest(userId, leaveRequestId) {
    ...
    return prisma.leaveRequest.update({
        where: { id: leaveRequestId },
        data: { status: LeaveStatus.CANCELLED, actionByUserId: userId, actionAt: new Date() },
    });
}
/**
 ...
 */
export async function approveLeaveRequest(callerRoles, callerId, leaveRequestId, actionNote) {
    const prisma = getPrisma();
    const leave = await prisma.leaveRequest.findUnique({
        where: { id: leaveRequestId },
        include: { user: true },
 ...
 */
export async function rejectLeaveRequest(callerRoles, callerId, leaveRequestId, actionNote) {
    const prisma = getPrisma();
    const leave = await prisma.leaveRequest.findUnique({
        where: { id: leaveRequestId },
        include: { user: true },
