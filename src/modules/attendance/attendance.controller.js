...
export async function punchIn(req, res) {
  ...
  const punch = await attendanceService.punchIn(
    req.user.sub,
    req.body.latitude,
    req.body.longitude,
    req.deviceId,
...
export async function myAttendanceOverview(req, res) {
  ...
  const result = await attendanceService.getUserAttendanceOverview(
    req.user.sub,
    startDate,
    endDate,
...
export async function webAttendanceOverview(req, res) {
  ...
  const result = await attendanceService.getWebAttendanceOverview(
    req.user.roles,
    req.user.sub,
    req.query,
  );
...
export async function webAttendanceRecords(req, res) {
  // Query is validated by middleware
  const result = await attendanceService.getWebAttendanceRecords(
    req.user.roles,
    req.user.sub,
    req.query,
  );
...
export async function upsertRegularization(req, res) {
  const result = await attendanceService.upsertRegularization(
    req.user.roles,
    req.user.sub,
    req.params.userId,
    req.params.date,
...
export async function deleteRegularization(req, res) {
  await attendanceService.deleteRegularization(
    req.user.roles,
    req.user.sub,
    req.params.userId,
    req.params.date,
