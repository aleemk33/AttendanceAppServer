...
export async function createDeviceChangeRequest(userId, data) {
  ...
  await prisma.deviceChangeRequest.updateMany({
    ...
    data: {
      ...
      actionAt: new Date(),
      actionNote: "Replaced by new request",
    },
  });
 ...
 */
export async function listDeviceChangeRequestsWeb(
  callerRoles,
  callerId,
  filters,
) {
 ...
 */
export async function approveDeviceChange(
  callerRoles,
  callerId,
  requestId,
  actionNote,
  ...
  if (!dcr) throw new NotFoundError("Device change request");
  ...
  const result = await prisma.$transaction(async (tx) => {
    ...
    await tx.attendanceProfile.upsert({
      ...
      create: {
        userId: dcr.userId,
        boundDeviceId: dcr.requestedDeviceId,
 ...
 */
export async function rejectDeviceChange(
  callerRoles,
  callerId,
  requestId,
  actionNote,
