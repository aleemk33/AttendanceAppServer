import { Role, LeaveStatus, DeviceChangeStatus, OverrideStatus } from '@prisma/client';
import { getPrisma } from '../../config/database.js';
import { env } from '../../config/env.js';
import { businessToday, businessYesterday, businessMonthStart, dateRange, isWeeklyOff, clampEndDate, } from '../../common/index.js';
// ─── Mobile Dashboard ────────────────────────────────────────────────────────
/**
...
export async function getMobileDashboard(userId) {
    ...
    if (todayPunch) {
        if (todayPunch.punchInAt && !todayPunch.punchOutAt) {
            todayStatus = { date: today, status: 'working', punchInAt: todayPunch.punchInAt.toISOString() };
        }
    ...
    const todayHoliday = await prisma.holiday.findFirst({
        where: { isDeleted: false, startDate: { lte: new Date(today) }, endDate: { gte: new Date(today) } },
    });
    if (todayHoliday) {
        todayStatus = { date: today, status: 'holiday', holiday: todayHoliday.title };
    }
    ...
    const summaryDates = dateRange(monthStart, appliedEndDate);
    const [punches, regularizations, leaves, holidays] = await Promise.all([
        prisma.attendancePunch.findMany({
            where: { userId, attendanceDate: { gte: new Date(monthStart), lte: new Date(appliedEndDate) } },
        }),
        ...
        prisma.holiday.findMany({
            where: { isDeleted: false, startDate: { lte: new Date(appliedEndDate) }, endDate: { gte: new Date(monthStart) } },
        }),
    ]);
    const punchMap = new Map(punches.map((p) => [p.attendanceDate.toISOString().slice(0, 10), p]));
    const regMap = new Map(regularizations.map((r) => [r.attendanceDate.toISOString().slice(0, 10), r]));
    // Expand holiday ranges into date-level lookup.
    const holidayDateSet = new Set();
    for (const h of holidays) {
        const hDates = dateRange(h.startDate.toISOString().slice(0, 10), h.endDate.toISOString().slice(0, 10));
    ...
    for (const l of leaves) {
        const lDates = dateRange(l.startDate.toISOString().slice(0, 10), l.endDate.toISOString().slice(0, 10));
        for (const d of lDates) {
            if (!isWeeklyOff(d) && !holidayDateSet.has(d))
                leaveDateSet.add(d);
        }
    }
    const { FULL_DAY_MINUTES } = env();
    let presentDays = 0, halfDays = 0, absentDays = 0, leaveDays = 0, holidayDays = 0, weeklyOffDays = 0;
    for (const date of summaryDates) {
        ...
        if (punch && punch.punchInAt && punch.punchOutAt) {
            if (punch.workedMinutes != null && punch.workedMinutes >= FULL_DAY_MINUTES)
                presentDays++;
            else
                halfDays++;
        }
    ...
    const attendancePercentage = denominator > 0 ? Math.round((numerator / denominator) * 10000) / 100 : 0;
    const monthSummary = { presentDays, halfDays, absentDays, leaveDays, holidayDays, weeklyOffDays, attendancePercentage };
    ...
    let count = 0;
    while (count < 7 && cursor >= '2020-01-01') {
        if (!isWeeklyOff(cursor) && !holidayDateSet.has(cursor)) {
            ...
            if (leaveDateSet.has(cursor)) {
                ...
                status = p.workedMinutes != null && p.workedMinutes >= FULL_DAY_MINUTES ? 'present' : 'halfDay';
                wm = p.workedMinutes;
            }
            else if (p) {
                status = 'halfDay';
 ...
 */
export async function getWebDashboard(callerRoles, callerId, startDate, endDate) {
    const prisma = getPrisma();
    const today = businessToday();
    ...
    const pendingLeaveCount = await prisma.leaveRequest.count({ where: pendingLeaveWhere });
    ...
    const pendingDeviceChangeCount = await prisma.deviceChangeRequest.count({ where: pendingDCWhere });
    // Upcoming holidays
    const upcomingHolidays = await prisma.holiday.findMany({
        where: { isDeleted: false, endDate: { gte: new Date(today) } },
        orderBy: { startDate: 'asc' },
    ...
    const [punches, regularizations, leaves, holidays] = await Promise.all([
        prisma.attendancePunch.findMany({
            where: { userId: { in: userIds }, attendanceDate: { gte: new Date(effectiveStart), lte: new Date(appliedEndDate) } },
        }),
        prisma.attendanceRegularization.findMany({
            where: { userId: { in: userIds }, attendanceDate: { gte: new Date(effectiveStart), lte: new Date(appliedEndDate) } },
        }),
        ...
        prisma.holiday.findMany({
            where: { isDeleted: false, startDate: { lte: new Date(appliedEndDate) }, endDate: { gte: new Date(effectiveStart) } },
        }),
    ]);
    // Shared holiday lookup for all users in selected date range.
    const holidayDateSet = new Set();
    for (const h of holidays) {
        const hd = dateRange(h.startDate.toISOString().slice(0, 10), h.endDate.toISOString().slice(0, 10));
        for (const d of hd)
            holidayDateSet.add(d);
    }
    const { FULL_DAY_MINUTES } = env();
    let totalPresent = 0, totalHalf = 0, totalAbsent = 0, totalLeave = 0, totalHoliday = 0, totalWeeklyOff = 0;
    // Aggregate per-user day states into organization/team totals.
    for (const uid of userIds) {
        const uPunches = new Map(punches.filter((p) => p.userId === uid).map((p) => [p.attendanceDate.toISOString().slice(0, 10), p]));
        const uRegs = new Map(regularizations.filter((r) => r.userId === uid).map((r) => [r.attendanceDate.toISOString().slice(0, 10), r]));
        const uLeaves = leaves.filter((l) => l.userId === uid);
        const uLeaveDates = new Set();
        for (const l of uLeaves) {
            const ld = dateRange(l.startDate.toISOString().slice(0, 10), l.endDate.toISOString().slice(0, 10));
            for (const d of ld)
                if (!isWeeklyOff(d) && !holidayDateSet.has(d))
                    uLeaveDates.add(d);
        }
    ...
    const attendancePercentage = denom > 0 ? Math.round(((totalPresent + totalHalf * 0.5) / denom) * 10000) / 100 : 0;
    return {
        range: { startDate: effectiveStart, endDate: rawEnd, appliedEndDate, currentDateExcluded },
        headcount,
        attendanceSummary: {
